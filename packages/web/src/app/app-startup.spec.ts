...
describe('initial shell loading', () => {
  it('shows the loading state while the initial route waits', async () => {
    ...
    fixture.detectChanges();
    expect(element.querySelector('app-startup-page')).toBeNull();
    decision.next(true);
    await navigation;
    ...
    expect(JSON.parse(transfer.toJson())['app-shell']).toBe('public');
  });

  it('uses the public shell after an initial redirect', async () => {
  ...
  it('retains the active shell while a later navigation waits or is cancelled', async () => {
    ...
    fixture.detectChanges();
    expect(fixture.componentInstance['publicPage']()).toBe(true);
    expect(fixture.componentInstance['startup']()).toBe('ready');
