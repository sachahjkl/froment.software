...
CREATE TABLE `__new_team_invitations` (
	...
	CONSTRAINT "team_invitation_profile_check" CHECK("profile" in ('collaborator', 'accountant', 'accounting-validator', 'accounting-reader') or (length("profile") = 33 and substr("profile", 1, 7) = 'custom:' and substr("profile", 8, 1) between '0' and '7' and substr("profile", 8) not glob '*[^0-9A-HJKMNP-TV-Z]*'))
);
...
CREATE TABLE `__new_team_members` (
	...
	CONSTRAINT "team_member_profile_check" CHECK("profile" in ('collaborator', 'accountant', 'accounting-validator', 'accounting-reader') or (length("profile") = 33 and substr("profile", 1, 7) = 'custom:' and substr("profile", 8, 1) between '0' and '7' and substr("profile", 8) not glob '*[^0-9A-HJKMNP-TV-Z]*')),
