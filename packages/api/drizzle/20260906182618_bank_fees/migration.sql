ALTER TABLE `bank_matches` ADD `fee_cents` integer DEFAULT 0 NOT NULL;--> statement-breakpoint
...
CREATE TABLE `__new_bank_matches` (
	...
	CONSTRAINT "bank_match_fee_check" CHECK("fee_cents" >= 0 and "fee_cents" < "amount_cents")
);
--> statement-breakpoint
INSERT INTO `__new_bank_matches`(`id`, `request_id`, `amount_cents`, `transaction_id`, `payment_id`, `matched_at`, `matched_by_user_id`, `cancelled_at`, `cancelled_by_user_id`, `cancellation_reason`) SELECT `id`, `request_id`, `amount_cents`, `transaction_id`, `payment_id`, `matched_at`, `matched_by_user_id`, `cancelled_at`, `cancelled_by_user_id`, `cancellation_reason` FROM `bank_matches`;--> statement-breakpoint
