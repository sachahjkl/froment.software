...
export async function checkCatalogWorkspace(page, testInfo) {
  ...
  let records = [
    ...
    { ...base, id: clientId, description: "Audit comptable", unitPriceCents: 7500 },
    {
      ...base,
      id: archivedId,
  ...
  try {
    ...
    await page.locator("#catalog-search").pressSequentially("developement");
    ...
    await page.locator("th.numeric [appTableSort]").click();
    ...
    await expect(page).toHaveURL(/q=Angular/);
    ...
    conflict = false;
    await page.locator('.catalog-form [type="submit"]').click();
    await expect(page).toHaveURL(/catalogue\/active\?q=Angular&sort=price-desc$/);
    ...
    await expect(page.locator("app-catalog tbody tr")).toHaveCount(2);
    await page.reload();
    await expect(page.locator("#catalog-search")).toHaveValue("Angular");
